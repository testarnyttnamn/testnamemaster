...
class cosmoinitTestCase(TestCase):
    ...
    def setUp(self):
        cosmology=Cosmology(cobaya_dic)
        self.H0check = 65.0
    ...
    def test_cosmo_init(self):
        emptflag = bool(cosmology.cosmo_dic)
        npt.assert_equal(emptflag, False,
                         err_msg='Cosmology dictionary not initialised '
                                 'correctly.')

    def test_cosmo_asign(self):
        cosmology.cosmo_dic['H0'] = self.H0check
        npt.assert_almost_equal(cosmology.cosmo_dic['H0'],
                                self.H0check,
